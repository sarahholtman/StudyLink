...
async function loadGroupNotifications() {
    ...
    if (!userId) {
        groupNotifications.innerHTML = "<p>User not logged in.</p>";
        return;
    }
    ...
    if (!response.ok) {
        groupNotifications.innerHTML = "<p>Could not load notifications.</p>";
        return;
    }
    ...
    notifications.slice(0, 10).forEach((notification) => {
        ...
        item.innerHTML = `
      <h3>${notification.type.replace("_", " ")}</h3>
    ...
    if (notifications.length > 10 && viewAllButton) {
...
function loadPlaceholderSessions() {
    ...
    if (sessions.length > 10 && viewAllButton) {
        viewAllButton.classList.remove("hidden-button");
    }
}
...
if (resourceForm) {
    resourceForm.addEventListener("submit", async function (event) {
        ...
        if (response.ok) {
            await fetch(`/api/notifications/group/${groupId}`, {
                method: "POST",
                ...
                body: JSON.stringify({
                    message: "A new resource has been shared in this study group.",
                    type: "GROUP_UPDATE",
                }),
            });
...
document
    ...
    .addEventListener("click", async function () {
        const response = await fetch(`/api/notifications/group/${groupId}`, {
            method: "POST",
            ...
            body: JSON.stringify({
                message: "A new study session has been created for this group.",
                type: "SESSION_REMINDER",
            }),
        });

        if (response.ok) {
            alert("Study session notification created.");
            loadGroupNotifications();
        } else {
